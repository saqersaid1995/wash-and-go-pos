    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        filtered.forEach((rule) => {
          const itemName = getItemName(rule);
          if (!grouped.has(itemName)) grouped.set(itemName, []);
          grouped.get(itemName)!.push(rule);
        });

        return (
          <div className="space-y-4">
            {Array.from(grouped.entries()).map(([itemName, itemRules]) => (
              <div key={itemName} className="border border-border rounded-lg overflow-hidden">
                <div className="bg-muted/50 px-4 py-2.5 border-b border-border flex items-center justify-between">
                  ...
                  <Badge variant="secondary" className="text-[0.6rem]">{itemRules.length} {itemRules.length === 1 ? "service" : "services"}</Badge>
                </div>
                <Table>
                  <TableHeader>
                    <TableRow>
